...
from common_functions.common_Unet_test import testatlas, testkits, nntestorgan, testprostate, testcardiac
    ...
    for iteration in range(int(args.numIteration)):
        ...
        if iteration % args.augupdate == 0:
            ...
            if not args.wotrmeta:
                ...
                unrolled_model = Generic_UNet(args.NumsInputChannel, base_num_features, args.NumsClass,
...
def validatecardiac(DatafileValFold, model, criterion, logging, epoch, Savename, args, NumsClass = 4):
    ...
    DSC, SENS, PREC = testcardiac(model, True, Savename + '/results/',
                            ImgsegmentSize=args.patch_size,
                            deepsupervision=args.deepsupervision, DatafileValFold=DatafileValFold, NumsClass = NumsClass)
