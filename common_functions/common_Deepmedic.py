import torch
import numpy as np
from utilities import SoftDiceLoss, accuracy, _concat, hessian_vector_product
from collections import OrderedDict, Counter
from models.Deepmedic import Deepmedic
from tensorboard_logger import log_value
from common_functions.common_Deepmedic_test import testatlas, testkits, testorgan, testprostate, testcardiac
import time
...
def train(sampling_results, sampling_results_val, sampling_results_val_samepatch, Augindex, Augindex_ts, Augindex_ts_samepatch, model, samplemodel, samplemodel_ts, criterion, 
    ...
    inputsubp1 = sampling_results[1]
    inputsubp2 = sampling_results[2]
    target = sampling_results[3]
    listr = sampling_results[4]
    clslist = sampling_results[5]
    ## grep validation samples
    inputnor_val = sampling_results_val[0]
    inputsubp1_val = sampling_results_val[1]
    inputsubp2_val = sampling_results_val[2]
    target_val = sampling_results_val[3]
    listr_val = sampling_results_val[4]
    clslist_val = sampling_results_val[5]
    ## grep validation samples with the same patch
    inputnor_val_samepatch = sampling_results_val_samepatch[0]
    inputsubp1_val_samepatch = sampling_results_val_samepatch[1]
    inputsubp2_val_samepatch = sampling_results_val_samepatch[2]
    target_val_samepatch = sampling_results_val_samepatch[3]
    ...
    for iteration in range(int(args.numIteration)):
        ...
        #     Img.append(inputnor_val_samepatch[k, 0, :, :, 18])
        #     Img.append(target_val_samepatch[k, :, :, 10])
        #     Img.append(inputsubp1_val_samepatch[k, 0, :, :, 11])
        #     Img.append(inputsubp2_val_samepatch[k, 0, :, :, 10])
        # show_fivecase(Img)

        # this is sampled from learned distribution, for normal training.
        ...
        inputsubp1pick = torch.tensor(inputsubp1[iteration * args.batch_size: (iteration + 1) * args.batch_size, :, :, :])
        inputsubp1_var = inputsubp1pick.float().cuda()
        inputsubp1_var = torch.autograd.Variable(inputsubp1_var)
        inputsubp2pick = torch.tensor(inputsubp2[iteration * args.batch_size: (iteration + 1) * args.batch_size, :, :, :])
        inputsubp2_var = inputsubp2pick.float().cuda()
        inputsubp2_var = torch.autograd.Variable(inputsubp2_var)

        ####################################################################
        # process with validation samples, this is sampled for the validation of learning training-time augmentation
        ...
        inputnor_valpick = torch.tensor(inputnor_val[iteration * args.batch_size_val: (iteration + 1) * args.batch_size_val, :, :, :, :])
        inputnor_val_var = inputnor_valpick.float().cuda()
        inputnor_val_var = torch.autograd.Variable(inputnor_val_var)
        inputsubp1_valpick = torch.tensor(inputsubp1_val[iteration * args.batch_size_val: (iteration + 1) * args.batch_size_val, :, :, :, :])
        inputsubp1_val_var = inputsubp1_valpick.float().cuda()
        inputsubp1_val_var = torch.autograd.Variable(inputsubp1_val_var)
        inputsubp2_valpick = torch.tensor(inputsubp2_val[iteration * args.batch_size_val: (iteration + 1) * args.batch_size_val, :, :, :, :])
        inputsubp2_val_var = inputsubp2_valpick.float().cuda()
        inputsubp2_val_var = torch.autograd.Variable(inputsubp2_val_var)

        ####################################################################
        # process with validation samples, this is sampled for learning test augmentation
        # this is sampled from random picks
        ...
        inputnor_val_samepatchpick = torch.tensor(inputnor_val_samepatch[iteration * (args.batch_size_val + 1): (iteration + 1) * (args.batch_size_val + 1), :, :, :, :])
        ...
        inputsubp1_val_samepatchpick = torch.tensor(inputsubp1_val_samepatch[iteration * (args.batch_size_val + 1): (iteration + 1) * (args.batch_size_val + 1), :, :, :, :])
        inputsubp1_val_samepatch_var = inputsubp1_val_samepatchpick.float().cuda()
        inputsubp1_val_samepatch_var = torch.autograd.Variable(inputsubp1_val_samepatch_var)
        inputsubp2_val_samepatchpick = torch.tensor(inputsubp2_val_samepatch[iteration * (args.batch_size_val + 1): (iteration + 1) * (args.batch_size_val + 1), :, :, :, :])
        inputsubp2_val_samepatch_var = inputsubp2_val_samepatchpick.float().cuda()
        inputsubp2_val_samepatch_var = torch.autograd.Variable(inputsubp2_val_samepatch_var)
        ...
        if iteration % args.augupdate == 0:
            ...
            if not args.wotrmeta:
                ...
                # compute output
                output = model(inputnor_var, inputsubp1_var, inputsubp2_var)
                ...
                for batch in list(range(inputnorpick.size()[0])):
                    ...
                    outputas = output[batch, :, :, :, :]
                    targets = target_var[batch, :, :, :]
                    # there is a problem, because it could not be the batch size
                    losssample += weightsk / inputnorpick.size()[0] * criterion(outputas.unsqueeze(0), targets.unsqueeze(0))

                gradients = torch.autograd.grad(losssample, fast_weights.values(), create_graph=True)  # if order = high, I need the graph

                # it can be updated with moment/weightdecay, to find the real direction. But it may not be necessary, as long as all I need is angel < 180
                ...
                unrolled_model = Deepmedic(args.NumsInputChannel, args.NumsClass, args.nores, args.simplearch, dropoutrate = args.dropoutrate)
                ...
                ## this is for the training-time augmentation learning
                outputval = unrolled_model(inputnor_val_var, inputsubp1_val_var, inputsubp2_val_var)

                if args.softdsc == 0:
                    lossval = criterion(outputval, target_val_var)
                if args.softdsc == 1:
                    lossval = SoftDiceLoss(outputval, target_val_var, DSCvalcls.copy(), batch_dice = True)
                if args.softdsc == 2:
                    lossval = criterion(outputval, target_val_var) + SoftDiceLoss(outputval, target_val_var, DSCvalcls.copy(), batch_dice = True)
                if args.softdsc == 3:
                    trainingproces = epoch / (args.epochs - 1)
                    lossval = (1 - trainingproces) * criterion(outputval, target_val_var) + trainingproces * SoftDiceLoss(outputval, target_val_var, DSCvalcls.copy(), batch_dice = True)

                lossvalce = criterion(outputval, target_val_var)
                lossvaldsc = SoftDiceLoss(outputval, target_val_var, list(range(args.NumsClass)), batch_dice = True)
                ...
                implicit_grads = hessian_vector_product(args, model, samplemodel, vector, inputnor_var, inputsubp1_var, inputsubp2_var, target_var, Augweightpick, Augindexpick, criterion, clslist, Augindex, iteration)
            ...
            if args.wotrmeta:
                # It is just for recording and indicator.
                outputval = unrolled_model(inputnor_val_var, inputsubp1_val_var, inputsubp2_val_var)
                if args.softdsc == 0:
                    lossval = criterion(outputval, target_val_var)
                if args.softdsc == 1:
            ...
            # ########################## for debuging ##########################
            # outputval = torch.zeros(10,2,21,21,21).cuda()
            # lossval = torch.zeros(1).cuda()
            # lossvalce = torch.zeros(1).cuda()
            # lossvaldsc = torch.zeros(1).cuda()
            # ##################################################################

            if not args.wotsmeta:
                ## test-time augmentation

                outputval_samepatch = unrolled_model(inputnor_val_samepatch_var, inputsubp1_val_samepatch_var, inputsubp2_val_samepatch_var)
                ...
                # I can use softdiceloss, which is easy to select cls
                '''
                ...
                # output_tta = torch.zeros(outputval_samepatch.shape[1:])
                ...
                for batch in list(range(inputnor_val_samepatch_var.size()[0] - 1)):
                    ...
                    outputas = outputval_samepatch[batch + 1, :, :, :, :]
                    ...
                    if (ttaindex > 0 and ttaindex <= 40) or ttaindex == 83:
                        ...
                        # Img = []
                        # for k in range(5) :
                        #     Img.append(inputnor_val_samepatchpick[k, 0, :, :, 18])
                        #     Img.append(target_val_samepatchpick[k, :, :, 10])
                        #     Img.append(outputas[1, :, :, 10].data.cpu().numpy())
                        #     Img.append(outputas_revert[1, :, :, 10])
                        # show_fivecase(Img)

                        outputas = torch.tensor(outputas_revert.copy())
            ...
            # logging.info('meta model backward ' + str(time.time() - point2) + ' seconds')  # 300s ? for the first time
            # point3 = time.time()
        ...
        if not args.woupdate:
            if args.wotrmeta or iteration % args.augupdate != 0:
                model.train()
                ## the model should be updated with learned data augmentation distribution.
                output = model(inputnor_var, inputsubp1_var, inputsubp2_var)
...
def adjust_learning_rate_arch_ts(optimizer_arch, epoch, args):
    """it is used by meta-task."""
    # and more to converge
    # from nnunet
    archlr = args.archts_learning_rate * (1 - epoch / args.epochs)**0.9
    # log to TensorBoard
    if args.tensorboard:
        log_value('learning_rate_arch_ts', archlr, epoch)
    for param_group in optimizer_arch.param_groups:
        param_group['lr'] = archlr

def validateatlas(DatafileValFold, model, criterion, logging, epoch, Savename, args):
    model.eval()

    DSC, SENS, PREC = testatlas(model, True, Savename + '/results/', DatafileValFold=DatafileValFold, trainval = False)
...
def validateprostate(DatafileValFold, model, criterion, logging, epoch, Savename, args, NumsClass = 2):
    ...
    DSC, SENS, PREC = testprostate(model, True, Savename + '/results/', DatafileValFold=DatafileValFold, trainval = False)
...
def validatecardiac(DatafileValFold, model, criterion, logging, epoch, Savename, args, NumsClass = 4):
    model.eval()

    DSC, SENS, PREC = testcardiac(model, True, Savename + '/results/', DatafileValFold=DatafileValFold, trainval = False)
...
def validatekits(DatafileValFold, model, criterion, logging, epoch, Savename, args):
    ...
    DSC, SENS, PREC = testkits(model, True, Savename + '/results/', DatafileValFold=DatafileValFold, trainval = False)
...
def validateorgan(DatafileValFold, model, criterion, logging, epoch, Savename, args):
    ...
    DSC, SENS, PREC = testorgan(model, True, Savename + '/results/', DatafileValFold=DatafileValFold, trainval = False)
